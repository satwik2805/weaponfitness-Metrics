...
await p.locator('input').first().fill('newfortimark@gmail.com'); await p.locator('input').nth(1).fill('Wf!vbu5XwEHjqE1W');
await p.getByRole('button',{name:'Sign in'}).last().click(); await p.waitForTimeout(9000);
await p.screenshot({path:'docs/design/review/owner2-dark.png'});
const t=p.getByLabel('Toggle theme').first(); if(await t.count()){await t.click(); await p.waitForTimeout(1500);}
await p.screenshot({path:'docs/design/review/owner2-light.png'});
