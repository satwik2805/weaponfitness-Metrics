...
await p.locator('input').first().fill('fortimark.demo.admin@gmail.com');
await p.locator('input').nth(1).fill('DemoAdmin!2026');
await p.getByRole('button', { name: 'Sign in' }).last().click();
await p.waitForTimeout(9000);
await p.screenshot({ path: 'docs/design/review/rf-admin.png' });
const t = p.getByLabel('Toggle theme').first();
if (await t.count()) { await t.click(); await p.waitForTimeout(1500); }
await p.screenshot({ path: 'docs/design/review/rf-admin-light.png' });
